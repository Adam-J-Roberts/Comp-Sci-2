...
int Animal::DogEatDog(char type)
{
		if (type == 'd' || type == 'D') {return 3;}
		if (type == 'c' || type == 'C') {return 2;}
		if (type == 'b' || type == 'B') {return 1;}
}
bool Animal::operator==(const Animal &one)
{
    if (entry_id == one.entry_id) {return true;}
    else {return false;}
}
bool Animal::operator!=(const Animal &one)
{
...
bool Animal::operator>(const Animal &one)
{
	if (DogEatDog(type) > DogEatDog(one.type)) {return true;}
	else {return false;}
}
bool Animal::operator<(const Animal &one)
{
...
ostream &operator<<(ostream& mystream, Animal &one)
{
		one.print_info();
		return mystream;
}
